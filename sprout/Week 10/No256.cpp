...
tuple<__int128_t,__int128_t,__int128_t> coeff(int x1, int y1, int x2, int y2) {
    __int128_t del = (__int128_t)x1*x2*(x1-x2);
    __int128_t del_a = (__int128_t)(y1*x2-x1*y2);
    __int128_t del_b = (__int128_t)(y2*x1*x1-y1*x2*x2);
...
int solve(vector <Dot> &p, vector <int> &dp,int cur) {
    ...
    for (int i = start+1; i < n; ++i) {//second dot
        if (!((cur>>i)&1)) continue;
        tuple<__int128_t,__int128_t,__int128_t> s_sol = coeff(p[start].x,p[start].y,p[i].x,p[i].y);
        ...
        for (int j = 0; j < n; ++j) {//others
            ...
            tuple<__int128_t,__int128_t,__int128_t> k_sol = coeff(p[start].x,p[start].y,p[j].x,p[j].y);
